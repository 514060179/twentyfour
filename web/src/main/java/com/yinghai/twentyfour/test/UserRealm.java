...
public class UserRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        ManagerUser member = managerUserService.findByName(token.getUsername());
        if(member != null){
//            if(member.getIslock() !=null && member.getIslock() == 1){
//                throw new AuthenticationException("msg:该已帐号禁止登录.");
//            }
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo("simon", member.getPassword().toCharArray(), ByteSource.Util.bytes("simon"),this.getName());
